...
class TrainDiffusiondiffogResWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        self.model.model = base_policy.model
        print("Loaded model from checkpoint")
        self.model.obs_encoder = base_policy.obs_encoder
        print("Loaded obs_encoder from checkpoint")
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        raw_loss_opt = self.model.compute_loss(batch)

                        raw_loss_opt.backward()

                        #torch.nn.utils.clip_grad_norm_(self.model.dto_layer.parameters(), max_norm=1.0)
                        ...
                        raw_loss_opt_cpu = raw_loss_opt.item()
                        training_losses_opt.append(raw_loss_opt_cpu)
                        step_log = {
                            'train_loss_opt': raw_loss_opt_cpu,
                ...
                train_loss_opt = np.mean(training_losses_opt)

                step_log['train_loss_opt'] = train_loss_opt
                ...
                if (self.epoch % cfg.training.rollout_every) == 0:
                    runner_log_original = env_runner.run_original(policy)
                    step_log.update(runner_log_original)
                    runner_log_diffog = env_runner.run_diffog(policy)
                    step_log.update(runner_log_diffog)
                    ...
                    with torch.no_grad():
                        ...
                        with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                            ...
                            for batch_idx, batch in enumerate(tepoch):
                                ...
                                val_losses_opt.append(raw_loss_opt)
                                if (cfg.training.max_val_steps is not None) \
                                    and batch_idx >= (cfg.training.max_val_steps-1):
                                    break
                        if len(val_losses_opt) > 0:
                            val_loss_opt = torch.mean(torch.tensor(val_losses_opt)).item()
                            # log epoch average validation loss
                            step_log['val_loss_opt'] = val_loss_opt
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        gt_action = gt_action[:, -pred_action.shape[1]:, :]
...
def main(cfg):
    workspace = TrainDiffusiondiffogResWorkspace(cfg)
    workspace.run()
